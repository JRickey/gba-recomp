//! A compact 8x8 bitmap font covering printable ASCII (0x20..=0x7E).
//!
//! Each glyph is eight bytes, one per row, MSB = leftmost pixel. This is
//! the classic IBM-PC-style 8x8 cell shape, authored here as plain data
//! so the play overlay carries no external font file (the project ships
//! zero bundled assets). Only the glyphs the menu actually draws need to
//! be legible; the full range is provided so any label renders.

/// Glyph cell dimensions, in source pixels.
pub const W: usize = 8;
pub const H: usize = 8;

/// Rows (8 bytes) for one printable character, or the `?` fallback for
...
/// 0x20..=0x7E, eight bytes per glyph, row-major, MSB-left.
#[rustfmt::skip]
static FONT: [[u8; 8]; 95] = [
    ...
    [0x00,0x00,0xFE,0xCC,0x18,0x66,0xFE,0x00], // 'z'
